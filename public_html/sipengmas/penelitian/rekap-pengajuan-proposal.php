...
$query = "SELECT * FROM pengajuan_penelitian WHERE 1";
...
$total_sql = !empty($tahun_filter) 
    ? "SELECT COUNT(*) AS total FROM pengajuan_penelitian WHERE tahun = '$tahun_filter'" 
    : "SELECT COUNT(*) AS total FROM pengajuan_penelitian";
$total_result = $conn->query($total_sql);
$total_row = $total_result->fetch_assoc();
$total_penelitian = $total_row['total'];

// Query untuk menghitung total seluruh penelitian
$total_all_sql = "SELECT COUNT(*) AS total FROM pengajuan_penelitian";
...
<!-- Download Button -->
    <div class="upload-button-container">
       ...
       <!-- Form Filter Tahun -->
       <form action="rekap-pengajuan-proposal.php" method="POST" class="mb-3">
            <div class="row">
                <div class="col-md-3">
                    ...
                    <select name="tahun" class="form-select" id="tahun">
                        ...
                        $query_tahun = "SELECT DISTINCT tahun FROM pengajuan_penelitian ORDER BY tahun DESC";
      ...
      <?php
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>
                            ...
                            <td>{$row['pendanaan']}</td>
                            <td>{$row['dana_disetujui']}</td>
                            <td>
                                <!-- Edit Form -->
                                <form action='edit_pengajuan.php' method='POST' style='display:inline;'>
                                    <input type='hidden' name='id' value='{$row['id']}'>
                                    <button type='submit' class='btn btn-warning btn-sm'>Edit</button>
                                </form>
                                <!-- Delete Form -->
                                <form action='delete_pengajuan.php' method='POST' style='display:inline;'>
                                    <input type='hidden' name='id' value='{$row['id']}'>
