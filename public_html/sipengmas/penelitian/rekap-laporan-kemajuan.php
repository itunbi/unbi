...
$query = "SELECT * FROM laporan_akhir WHERE 1";
...
$total_sql = !empty($tahun_filter) 
    ? "SELECT COUNT(*) AS total FROM laporan_akhir WHERE tahun = '$tahun_filter'" 
    : "SELECT COUNT(*) AS total FROM laporan_akhir";
$total_result = $conn->query($total_sql);
$total_row = $total_result->fetch_assoc();
$total_penelitian = $total_row['total'];

// Query untuk menghitung total seluruh penelitian
$total_all_sql = "SELECT COUNT(*) AS total FROM laporan_akhir";
...
<body>
    <div class="container-fluid">
      ...
      <div class="accordion-item">
        ...
        <div id="collapseThree" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordionExample">
          <div class="accordion-body">
            <ul class="list-unstyled">
              <li><a href="rekap-laporan-akhir.php">Penelitian</a></li>
            ...
            <h2 class="mb-4">Form Rekap Laporan <br> Kemajuan Penelitian</h2>
            <!-- Navbar -->
  <nav class="navbar navbar-expand-lg navbar-custom">
    <div class="container">
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav ms-auto">
          <li class="nav-item">
            <a class="nav-link" href="rekap-pengajuan-proposal.php">Pengajuan Proposal</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="rekap-monev.php">Hasil Review</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="rekap-laporan-kemajuan.php">Laporan Kemajuan</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="rekap_laporan_akhir.php">Laporan Akhir</a>
          </li>
        </ul>
      </div>
    </div>
    ...
    <!-- Form Filter Tahun -->
    <form action="rekap-laporan-kemajuan.php" method="POST" class="mb-3">
            <div class="row">
                <div class="col-md-3">
                    ...
                    <select name="tahun" class="form-select" id="tahun">
                        ...
                        $query_tahun = "SELECT DISTINCT tahun FROM kemajuan_penelitian ORDER BY tahun DESC";
...
<table class="table table-striped table-bordered">
      <thead class="table-dark">
        <tr>
          ...
          <th>NIDN Ketua Peneliti</th>
          <th>Anggota</th>
          <th>Skema Penelitian</th>
          <th>Judul Penelitian</th>
          <th>Kemajuan Penelitian</th>
          <th>Luaran</th>
      ...
      <?php
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>
                            ...
                            <td>{$row['fakultas']}</td>
                            <td>{$row['ketua_peneliti']}</td>
                            <td>{$row['nidn']}</td>
                            <td>{$row['anggota']}</td>
                            <td>{$row['skema_penelitian']}</td>
                            <td>{$row['judul_penelitian']}</td>
                             <td>{$row['kemajuan_penelitian']}</td>
                             <td>{$row['luaran']}</td>
                             <td>{$row['kendala']}</td>
                            ...
                            <td>
                                <!-- Edit Form -->
                                <form action='edit_rekap_kemajuan.php' method='POST' style='display:inline;'>
                                    <input type='hidden' name='id' value='{$row['id']}'>
                                    <button type='submit' class='btn btn-warning btn-sm'>Edit</button>
                                </form>
                                <!-- Delete Form -->
                                <form action='delete_rekap_pengajuan.php' method='POST' style='display:inline;'>
                                    <input type='hidden' name='id' value='{$row['id']}'>
     ...
     <div class="container mt-5">
        ...
        <!--total_penelitian-->
        <div class="mb-3">
            <h5>Total Penelitian <?php echo $tahun_filter ? "Tahun $tahun_filter" : "Semua Tahun"; ?></h5>
            <div class="progress">
                <div class="progress-bar" role="progressbar" style="width: <?php echo ($total_penelitian / $total_all_penelitian) * 100; ?>%;" aria-valuenow="<?php echo $total_penelitian; ?>" aria-valuemin="0" aria-valuemax="<?php echo $total_all_penelitian; ?>">
                    <?php echo $total_penelitian; ?> Penelitian
                </div>
            </div>
        </div>
  </div>
