...
$query = "SELECT * FROM hasil_monev WHERE 1";
...
$total_sql = !empty($tahun_filter) 
    ? "SELECT COUNT(*) AS total FROM hasil_monev WHERE tahun = '$tahun_filter'" 
    : "SELECT COUNT(*) AS total FROM hasil_monev";
$total_result = $conn->query($total_sql);
$total_row = $total_result->fetch_assoc();
$total_penelitian = $total_row['total'];

// Query untuk menghitung total seluruh penelitian
$total_all_sql = "SELECT COUNT(*) AS total FROM hasil_monev";
      ...
      <div class="accordion-item">
        ...
        <div id="collapseThree" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordionExample">
          <div class="accordion-body">
            ...
            <h2 class="mb-4">Form Rekap Review <br> Pengajuan Proposal</h2>
            <!-- Navbar -->
...
<!-- Download Button -->
    <div class="upload-button-container">
       ...
       <!-- Form Filter Tahun -->
       <form action="rekap-monev.php" method="POST" class="mb-3">
            <div class="row">
                <div class="col-md-3">
      ...
      // Menampilkan data dari database dalam tabel
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>
                            ...
                            <td>{$row['skema_hibah']}</td>
                            <td>{$row['nama_review']}</td>
                            <td>{$row['hasil_review']}</td>
                            <td>{$row['tindak_lanjut']}</td>
                            <td>{$row['link_file']}</td>
                            <td>
                                <!-- Edit Form -->
                                <form action='edit_rekap_monev.php' method='POST' style='display:inline;'>
                                    <input type='hidden' name='id' value='{$row['id']}'>
                                    <button type='submit' class='btn btn-warning btn-sm'>Edit</button>
                                </form>
                                <!-- Delete Form -->
                                <form action='delete_hasil_monev.php' method='POST' style='display:inline;'>
                                    <input type='hidden' name='id' value='{$row['id']}'>
