<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Struktur Akademik</title>
    <link rel="stylesheet" href="struktur.css">
    <link rel="icon" type="image/png" href="Image/Unbi.png">
</head>
<body>

    <nav class="navbar">
        <div class="navbar-container">
            <div class="navbar-brand">Universitas Bali Internasional</div>
            <ul class="navbar-menu">
                <li><a href="/Psikologi">Beranda</a></li>
            </ul>
        </div>
    </nav>


    <!-- Header Section -->
    <header class="header">
        <div class="header-content">
            <h1>Struktur Akademik</h1>
        </div>
    </header>


    <section class="struktur-section">
        <div class="struktur-container">
            <h2>Struktur Akademik</h2>
            <img src="Image/struktura.jpg" alt="Struktur Akademik Universitas Bali Internasional">
        </div>
    </section>
    
  
    <!-- Footer -->
    <footer class="footer">
        <p>&copy; 2024 Universitas Bali Internasional. Dibangun dengan dedikasi tinggi.</p>
    </footer>

    <script src="slider.js"></script>
</body>
</html>
